...
**最優先タスク**：**テキストトランジション（slice-ts スクロール演出）を実装**

経緯：
- 2026-04-26 に [依頼ページ](https://www.notion.so/34d980e4ee138198a2b3e501f696ec50) の 5 番として起票された
- beta 担当のカルディ２が「優先度低」とラベル付けたが、**やっさん本人は優先度を下げたつもりなし**（明示的に発言）
- 私が「優先度低と書かれていた」を理由に保留したのは、やっさんの意向を確認しなかった怠慢

実装のための情報：
- overlay 掲示板の「カルディ２同士直接メモ」セクションの「依頼 3」で A〜E の仕様質問を beta に投げてある
- beta 側で `data-slice-ts` 属性付きの段落で送る設計が部分実装されている（captions.js v0.13.31）
- まだ仕様詳細（フォーマット、ts 値の型、新規判定ロジック等）が beta から返ってきていない可能性あり
- **次セッションの最初に：beta の captions.js v0.13.31+ を直接 Read して slice-ts の送信実装を確認、そこから仕様を逆引きして overlay 側に組み込む**
