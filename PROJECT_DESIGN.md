...
#### 経緯
v0.3.21 → v0.3.22 でしきい値方式 → クールダウン方式と微調整したが、やっさん検証で

- 縦 padding は効くが横 padding と角丸が一定領域から効かなくなる
- 文字入れ替わりでブルブル震える
- 行間・ブロック間隔をいじると角丸が無くなって戻せない

と続いた。やっさんから核心的な発言：「**そもそも文字サイズに合わせて背景の大きさを調整、ってのが難しいのかな。div の入れ子の調整の要領と思って簡単に考えてたけど・・・**」

これが正論。Web の常識通りの実装にすれば良かった。私が v0.3.9 で「ウィンドウサイズを動的に `.caption` に追従させる」設計を選んだ時点で、ResizeObserver と set_size のフィードバックループとの闘いが始まり、v0.3.10〜v0.3.22 で 13 段階の試行錯誤を費やした。

#### 修正
- `caption_resized` Tauri command を撤去（generate_handler からも削除）
- `setupResizeObserver` 関数撤去
- `WINDOW_PADDING_PX` / `RESIZE_COOLDOWN_MS` / `LAST_RESIZE_TIME` 撤去
- `WINDOW_HEIGHT_RATIO` を 0.5 に固定（モニタ高さの半分、字幕の伸び代）
- ウィンドウサイズは起動時固定、`set_monitor` で別モニタに移る時のみ再配置
- `.caption` は CSS の `inline-block + flex-shrink: 0 + flex-end` で自然に伸び縮み

#### 副作用（受け入れた）
- **クリックスルー OFF 中、字幕より上の透明領域でマウス操作が下のアプリに届かない**（v0.3.8 の問題が戻った）
- これは v0.4.x で **mouse polling 方式** で動的に `set_ignore_cursor_events` を切り替える設計で正攻法対応する
  - Rust 側で 50ms 周期でマウス位置を取得
  - WebView から `.caption` の bounding rect を Rust に push
  - マウスが `.caption` 領域内なら ignore=false、外なら ignore=true
  - これでウィンドウは大きく取りつつ、字幕の上だけ反応する

#### 学び（私の反省）
- **複雑なフィードバックループが起きる設計を選んだ**：v0.3.9 で「動的ウィンドウサイズ」を導入した時、ResizeObserver と set_size の循環依存を見抜けなかった
- **やっさんの「div の入れ子で簡単」発言を初めから聞くべき**だった。やっさんは Web の常識をベースに考えていて、それは正しい設計感覚
- **段階的実装の原則を破った**：v0.3.9 で大きな設計変更を入れた後、v0.3.10〜v0.3.22 で「症状を 1 つずつ追いかけて修正」ばかりして、根本設計の見直しに踏み切れなかった
- **「妥協に導かない」を破った**：v0.3.20 のデバッグスクショで「赤枠 + 角丸が一瞬見える、すぐ bottom 以外消える」を見て、やっさんは答えを示してくれていた。私はそれをすぐに振動ループと結びつけられず、しきい値・クールダウンと小手先で対応した

#### 次セッションへの引き継ぎ（カルディ２へ）

**最優先タスク**：**テキストトランジション（YouTube 風ストリーミングモード）を実装**

#### やっさんが想像している UX
2026-04-27 のやり取りで、やっさんからインスパイア元のスクショ + 補足発言：

- **インスパイア元**：YouTube 動画の自動字幕（CC）
- **動き**：
  1. 文字が 1 つずつ追加されていく（タイプライター効果、1 行の中で右に伸びる）
  2. 行が一定数溜まったら、古い行が**上に押し出され**、下から新規行が出てくる
  3. 途切れず流れ続ける、遅延なし
- やっさん発言：「ストリーミングモードも復活して、youtube みたいに一文字ずつ出てしばらくしたら上に押し出す形で入れ替わるモードもつけたい」「トランジションはこの youtube の字幕の出方からのインスパイア」

#### 経緯
- 2026-04-26 に [依頼ページ](https://www.notion.so/34d980e4ee138198a2b3e501f696ec50) の 5 番として起票された
- beta 担当のカルディ２が「優先度低」とラベル付けたが、**やっさん本人は優先度を下げたつもりなし**（明示発言）
- 私が「優先度低と書かれていた」を理由に保留したのは、やっさんの意向を確認しなかった怠慢

#### 実装の構成要素（次セッションのカルディ２へ）

1. **slice 受信構造**：beta が `slices: [{ts, text}]` 形式で「現在見えているべき行」を毎回送る or 差分送る
   - 仕様詳細は overlay 掲示板の「依頼 3」A〜E にこちらから問い合わせ済み（beta 担当からの返事待ち、または beta コードを直接 Read して逆引き）
   - beta v0.13.31+ で `data-slice-ts` 属性付きの段落送信が部分実装されている
2. **タイプライター効果**：受信した slice の text を 1 文字ずつ DOM に追加
   - JS `setInterval` で 30〜80ms 間隔、または CSS animation `@keyframes typing` で `width` を 0 → 100% に変化
   - 改行タイミングや句読点の間は微調整
3. **行スクロール**：DOM 内の `<p>` 要素を追加 → 古い `<p>` を CSS `transform: translateY(-行高)` で上に流す → 流し終わったら DOM 削除
   - CSS animation で滑らかに、duration 200〜300ms

#### 設計判断のヒント

- v0.3.6 の `transition: "scroll-up"` は「全文 fade/slide」レベル。YouTube 風の精度には届いていない、別次元の機能
- **既存 transition と排他**：YouTube 風モードは `transition` ではなく `streaming-mode` 等の新フィールドで切り替える
- **設計順**：
  1. まず beta 側の slice 送信仕様を確定（依頼 3 への返事 or beta コード直読み）
  2. overlay で slice 受信ロジック（差分追加、古い slice 削除）
  3. CSS で「下から fade-in / slide-up」アニメーション、行スクロールも CSS animation で
  4. タイプライター効果は最後の polish

**マウス問題の正攻法対応（v0.4.x）**：
- 現状：クリックスルー OFF 中、字幕より上の透明領域でマウス操作が下に届かない
- 解決策：mouse polling で動的に `set_ignore_cursor_events` を切り替え（Rust 側 50ms 周期）
- WebView から `.caption` の bounding rect を Rust に push する仕組みも要る
- これも次セッションでやる
